...
def search_flare_data(fl_search, event_type, condition):
    """
    Searches for flare data.
...
def process_data(data, origin):
    ...
    data = data.drop_duplicates(['event_peaktime', 'fl_goescls']).copy()
    
    # Add a new column 'Origin' with the provided origin value using .loc
    data.loc[:, 'Origin'] = origin
...
def process_directory(input_dir, hek_dir):
    """
    ...
    data_SSW = search_flare_data(fl_search, "FL", a.hek.FRM.Name == "SSW Latest Events")
    data_GOES = search_flare_data(fl_search, "FL", a.hek.OBS.Observatory == "GOES")
    ...
    data_full = data_full.drop_duplicates(['event_starttime', 'fl_goescls'], keep = "last")
    data_full = data_full.sort_values(by = ['event_starttime'])
    data_full = data_full.reset_index(drop = True)
    # Save the full data to a CSV file
    data_full.to_csv(hek_dir)
    ...
    # Number of flares in period
    print("Number of flares in period: ", len(data1))

    # Save the final data to a CSV file
    data1.to_csv(hek_dir)